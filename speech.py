import json
import logging
import operator
import re

import markovify
import spacy
from attrdict import AttrDict
from spacy_cld import LanguageDetector

import dbhelper
from dbhelper import get_model_chat_id, upsert_model_chat_id

logger = logging.getLogger(__name__)


def load_nlp_models(languages=None):
    if languages is None:
        languages = []
    nlp_models = None
    if languages:
        nlp_models = AttrDict({'languages': [], 'processors': []})
        ld = LanguageDetector()
        for lang in languages:
            try:
                model = spacy.load(lang)
            except OSError:
                logger.error(f'{lang} spacy module not found')
                continue
            model.add_pipe(ld)
            nlp_models['languages'].append(lang)
            nlp_models['processors'].append((lang, model))
        if not nlp_models['languages']:
            nlp_models = None
    return nlp_models


nlp = load_nlp_models()


def process_text(text):
    logger.info('performing n.l.p.')
    if not nlp:
        return text
    lang, proc = nlp.processors[0]
    doc = proc(text)
    scores = doc._.language_scores.items()
    if scores:
        guess = max(scores, key=operator.itemgetter(1))[0]
        if guess != lang and guess in nlp.languages:
            lang, proc = nlp.processors[nlp.languages.index(guess)]
            doc = proc(text)
    return doc


class PosifiedText(markovify.NewlineText):
    def word_split(self, sentence):
        logger.info('spliting sentece into words')
        return ['::'.join((w.text, w.pos_, w.dep_))
                for w in process_text(sentence)]

    def word_join(self, words):
        logger.info('joining words into sentence')
        sentence = ' '.join(w.split('::')[0] for w in words)
        return re.sub(r'\s([!?.,;"](?:\s|$))', r'\1', sentence)


def new_model(text):
    logger.info('creating a new model')
    Cls = PosifiedText if '' else markovify.NewlineText
    model = None
    try:
        model = Cls(re.sub(r'["\']', '', text),
                    retain_original=True)
    except KeyError:
        logger.error(f'cannot create a chain from {text}')
    return model


def get_model(chat):
    logger.info(f'fetching model for chat-id:{chat.id}')
    chat_id = str(chat.id)
    chat_data = get_model_chat_id(chat_id) or {}
    if 'chain' in chat_data:
        Cls = PosifiedText if '' else markovify.NewlineText
        chain = json.loads(chat_data["chain"])
        return Cls.from_chain(chain, corpus=chat_data["text"])


def update_model(chat, message):
    if not chat or not hasattr(chat, 'id'):
        raise ValueError('invalid type for chat')
    if not message:
        raise ValueError('message cannot be empty')
    logger.info(f'updating model for chat-id:{chat.id}')
    model = new_model(message)
    if not model:
        return
    else:
        model = model.chain
    chat_id = str(chat.id)
    chat_data = get_model_chat_id(chat_id) or {}
    if 'text' in chat_data:
        text = '\n'.join([chat_data["text"], message])
    else:
        text = '\n'.join(['', message])
    if 'chain' in chat_data:
        text = '\n'.join(
            text.splitlines()[-10000:])
        model = new_model(text).chain
    upsert_model_chat_id(chat_id, text, json.dumps(model.to_json()))


def delete_model(chat):
    logger.info(f'deleting model for chat-id:{chat.id}')
    chat_id = str(chat.id)
    dbhelper.delete_model(chat_id)


def new_message(chat):
    logger.info(f'generating message for chat-id:{chat.id}')
    model = get_model(chat)
    message = None
    if model:
        message = model.make_sentence(
            max_overlap_ratio=0.7,
            state_size=1,
            tries=1000
        )
    else:
        print("no model")
    return message or 'i need more data'
